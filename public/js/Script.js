...
const createMovieList = () => {
	...
	movieItems.forEach(element => {
		...
		<h4>${element.title}</h4>
		<p>Release Date : <span id="year">${element.releaseDate}</span></p>
		<button
		onclick="addFavourite(${element.id})" type="button" class="btn btn-success">
		Add to Favourites
		</button>
		</div>
		`;
...
const createFavouriteList = () => {
	...
	favItems.forEach(element => {
		...
		<img src="${element.posterPath}" class="img-fluid pb-2" alt="Responsive image">
		<h4>${element.title}</h4>
		<p>Relese Date : <span id="year">${element.releaseDate}</span></p>
        <button
		onclick="deletefromfavourites(${element.id})" type="button" class="btn btn-danger">
		Remove
		</button>
		</div>
		`;
